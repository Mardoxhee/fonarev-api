...
exports.createDiection = async (req, res) => {
  try {
...
exports.getAllDirections = async (req, res) => {
  try {
    const features = new APIfeatures(Direction.find(), req.query)
      ...
      .limitFields()
      .paginate();
    const directions = await features.query.populate(["account", "agent"]);
...
exports.getOneDirection = async (req, res) => {
  try {
    const direction = await Direction.findById(req.params.id).populate("agent");
    res.status(200).json({
      status: "success",
      direction,
    });
